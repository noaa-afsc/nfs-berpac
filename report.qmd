...
definitely_cu_crw_argos = function(.data, ...){

  possibly_func = purrr::possibly(cu_crw_argos, otherwise = NULL)

  result = NULL
  try_number = 1

  while(is.null(result) && try_number <= 20){
    try_number = try_number + 1
    result = possibly_func(.data, crw_control = list(method="L-BFGS-B"), ...)
  }
...
locs_fit <- locs %>% 
  ...
  cu_add_argos_cols() %>% 
  group_by(deployid) %>% 
  group_map(~definitely_cu_crw_argos(.x), 
            .keep = TRUE)
...
ecd_data <- data_list$ecdf %>% 
  ...
  dplyr::select(c(deployid,start:percent_dry,shallow_ecdf, deep_ecdf,full_ecdf,spline_interp,depth50pct, n_dives)) %>% 
